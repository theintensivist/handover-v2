...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!apiKey) {
    return res.status(500).json({
      error: "Gemini API key is missing. Please enter your custom Gemini API key in the UI, or configure your GEMINI_API_KEY in the Vercel environment variables."
    });
  }
